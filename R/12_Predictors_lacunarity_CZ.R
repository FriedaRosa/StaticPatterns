...
expanded_sf <- expanded %>% bind_rows() %>%
  right_join(
...
#----------------------------------------------------------#
# 2. Create country templates for rasterizing  -----
#----------------------------------------------------------#
...
# make templates for each dataset (in this case it's always a list with 1 element)
...
for (i in seq_along(species_list)) {
  ...
  atlas_i <-
    current_sf$datasetID[1] # similar to unique()

  template_i <-
    masked_templates[[1]] # 1 because we are doing it separately and the list only has 1 entry

  # Convert sf to raster
  r <-
    rasterize(current_sf, template_i, field = "presence")

  # Store masked raster
  masked_rasters[[paste(current_sf$samplingPeriodID[1], current_sf$verbatimIdentification[1], sep = "_")]] <-
    r
...
str(masked_raster_stack)
#----------------------------------------------------------#

# Save the raster stack
writeRaster(masked_raster_stack, filename = "Data/output/CZ_species_raster_stack.tif", overwrite = TRUE)
